...
class DEXRT_SOTA_Dataset(Dataset):
...
class MobileNetV3_DEXRT(nn.Module):
    def __init__(self, num_classes=2):
        ...
        self.backbone.classifier = nn.Sequential(
            nn.Linear(in_features, 512), nn.Hardswish(), nn.Dropout(0.2), nn.Linear(512, num_classes)
        )
    ...
    def forward(self, h_raw, l_raw):
        diff = torch.log(h_raw + 1e-6) - torch.log(l_raw + 1e-6)
        x = torch.cat([h_raw, l_raw, diff], dim=1)
...
def train_one_seed(args):
    ...
    train_loader = DataLoader(
        DEXRT_SOTA_Dataset(args.csv_path, f_map, "train"),
        batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, pin_memory=True
    )
    val_loader = DataLoader(
        DEXRT_SOTA_Dataset(args.csv_path, f_map, "val"),
        batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, pin_memory=True
    )

    model = MobileNetV3_DEXRT().to(device)
    optimizer = optim.AdamW(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    criterion = nn.CrossEntropyLoss(weight=torch.tensor([1.0, 1.5], device=device))
    scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.epochs)

    best_f1 = -1.0
    best_path = os.path.join(args.outdir, f"best_model_MOBILE_SOTA_seed{args.seed}.pth")

    for epoch in range(args.epochs):
        model.train()
        total_loss = 0.0
        loop = tqdm(train_loader, desc=f"Seed {args.seed} | MobileV3 Epoch {epoch + 1}/{args.epochs}")
        for h, l, y in loop:
        ...
        if f1 > best_f1:
            ...
            print(f"Saved best MobileV3 weight: {best_path}")

    print(f"Finished MobileV3 seed {args.seed}. Best Val F1: {best_f1:.4f}")
